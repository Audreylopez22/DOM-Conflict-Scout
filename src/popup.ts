...
const updateCountdown = () => {
  if (sessionEndDate) {
    ...
    if (timeLeft <= 0) {
      if (statsEl) statsEl.innerText = "Session finished. You can start a new one.";
      stopAllCountdowns();
      return;
    }
    ...
    if (statsEl) {
      statsEl.innerText = `🚨 Audit active. Ends in ${minutes}m ${seconds.toString().padStart(2, '0')}s`;
    }
  } else if (document.getElementById('stop-controls')?.style.display.includes('block')) {
      if (statsEl) statsEl.innerText = `🛰️ Audit active...`;
  }
};
...
chrome.runtime.onMessage.addListener((message) => {
  if (message.action === 'UPDATE_STATS') {
    ...
    if (statsEl) statsEl.innerText = "Session finished.";
